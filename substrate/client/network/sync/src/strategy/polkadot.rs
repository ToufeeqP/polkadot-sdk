	...
	pub fn new(
		...
		if should_start_with_warp {
			let warp_sync_config = warp_sync_config
				.expect("Warp sync configuration must be supplied when starting with warp sync.");
...
mod test {
	...
	#[test]
	fn avail_light_prefers_warp_when_available() {
	...
	#[test]
	fn avail_light_uses_chain_sync_without_warp_provider() {
		let client = Arc::new(TestClientBuilder::new().build());
		let strategy =
