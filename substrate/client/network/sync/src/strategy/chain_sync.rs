		...
		+ 'static,
{
	...
	fn reset_sync_start_point(&mut self) -> Result<(), ClientError> {
		...
		if let Some(BlockGap { start, end, .. }) = info.block_gap {
			if matches!(self.mode, ChainSyncMode::AvailLight) {
				// `AvailLight` uses the warp bootstrap target as its live anchor and intentionally
				// does not backfill historical ancestry before becoming useful.
				debug!(
					target: LOG_TARGET,
					"Ignoring block gap #{start} - #{end} for avail-light live sync."
				);
				self.gap_sync = None;
			} else {
